...
基本体系語彙は [06_vocabulary_basic_systems.md](./06_vocabulary_basic_systems.md) で管理する。数字は採用済みとし、方向・位置、色、数量・程度、時間・日付の基礎は、後の決定で採用されるまでは草案として扱う。
...
TODO: v1.1 で程度語が動作句も修飾できるかを決める。

### 6A.5 時間・日付の基礎

草案の時間語は、文頭に置いて文全体の時間文脈を示せる。
...
TODO: 正式な時間文脈に `la` を使うべきかを決める。完全な日付形式と時刻形式は v1.1 では未確定とする。
