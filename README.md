# AI時代の学習用人工言語Alimidaプロジェクト

## Alimidaとは

**Alimida**（略式: **Alim**）は、AI時代の言語学習を支援するための人工言語です。

本言語は、英語の代替そのものではなく、多言語学習の足場として、初学者が文構造・品詞・時制・語順を理解しやすくすることを目指します。AI添削、翻訳比較、発音練習と相性の良い、規則的で扱いやすい仕様を重視します。

## リポジトリについて

このリポジトリは、AI時代の学習に適した人工言語の仕様を管理するためのプロジェクトです。

## 現在のバージョン

- 発音仕様: v1.1
- 基本語彙: v1.1（100+α）
- 文法仕様: v1.1

## 読む順番

1. [Documentation Index](docs/00_index.md)
2. [Overview](docs/01_overview.md)
3. [Phonology / Pronunciation](docs/02_phonology.md)
4. [Grammar](docs/03_grammar.md)
5. [Core Vocabulary 100+α](docs/04_vocabulary_core100.md)
6. [Basic Systems Vocabulary](docs/06_vocabulary_basic_systems.md)
7. [Examples](docs/05_examples.md)

## 学習教材

v1.2 作業では、仕様書に加えて、教材用の例文・レッスン・練習問題を整備します。

- [Learning Examples](docs/10_learning_examples.md)
- [Lessons](docs/11_lessons.md)
- [Vocabulary Levels](docs/12_vocabulary_levels.md)
- [Practice](docs/13_practice.md)
- [Relation Words](docs/14_relation_words.md)

TODO・未決定事項は [TODO / Open Issues](docs/91_todo.md) に集約します。旧文書は [archive](archive/) に保管し、最新版仕様としては扱いません。
